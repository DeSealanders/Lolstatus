 ...
 * The account is identified by a config, which needs to be set before sending tweets
 */
...
class TweetManager {
    ...
    private function sendTweet($tweet) {
        ...
        if(isLive()) {
            ...
            Logger::getInstance()->log('General: Sent a tweet: ' . $tweet);
            if(isset($reply->errors)) {
                foreach($reply->errors as $error) {
                    Logger::getInstance()->log('Error while tweeting: ' . $error->message);
                }
                return false;
            }
        }
        else {
            echo '<p>Not sending tweet: <strong>' . $tweet .'</strong></p>';
        }
        return true;
    }
